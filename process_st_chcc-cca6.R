...
spatial_list <- sapply(slide_list,function(slide){
  print(slide)
  raw_data_directory <- paste0(PATH_TO_DATA,slide,"/outs/")
...
library(Seurat, lib.loc=PATH_RESULTS)
for (ids in c(1:6)) {
  jpeg(file=paste0(res_folder,Sys.Date(),"_dl_predictive_patches_A",ids,".jpeg"),width=960,height=960,quality = 100)
  # this function is not working with Seurat 4.1.1 for group other than ident
  print(SpatialPlot(spatial_list[[ids]], group.by=c('G100'),cols=c("HCC_High" = "blue", "Non_Info" = "grey90", "DL_NA" = "grey90", "ICCA_High" = "red")))
...
# back to seurat version 4.1.1 for MAST
detach("package:Seurat", unload=TRUE)
library(Seurat)

spatial_list_DEA <- lapply(c(1:6),function(y){
  x <- spatial_list[[y]]
  MAST <- FindMarkers(x,group.by = "G100",ident.1 = 'ICCA_High',ident.2 = 'HCC_High',test.use = "MAST", logfc.threshold=0)
})
...
for (ids in c(1:6)) {
  ...
  write.csv(diff_exp, file=paste0(res_folder, '/',Sys.Date(),'_MAST_A', ids, '.csv'))
  ...
  diff_exp<- diff_exp %>%
    mutate(TH= case_when(diff_exp$avg_log2FC>0 & diff_exp$p_val_adj<0.05 ~"Significantly UP",
                         ...
                         diff_exp$p_val_adj>0.05~"no DEGs"))
  ...
  if (ids == 2) {
    selectGenes = c('S100A6', 'AQP1', 'S100A14', 'KRT7', 'EPCAM', 'IGHG2', 'IGKC', 'IGHA1')
    diff_exp[match(selectGenes,rownames(diff_exp)),"lab" ]= selectGenes
  } else if (ids == 4) {
    selectGenes = c('S100A6', 'EPCAM', 'HNF1B', 'ANXA2', 'FLNA', 'ANXA11', 'ALB', 'FABP1', 'APOB', 'APOE', 'A2M', 'APOC2')
    diff_exp[match(selectGenes,rownames(diff_exp)),"lab" ]= selectGenes
  } else {
    selectGenes = NA
    diff_exp$lab= NA
  ...
  ## Plot Volcano plot
  print(paste0(res_folder,"Volcano_Hot_Cold_A",ids,".png"))
  png(paste0(res_folder,"Volcano_Hot_Cold_A",ids,".png"),width = 2000, height = 2000, res=300)
...
# Draw heatmaps for selected markers
for (gene in c('S100A6', 'AQP1', 'S100A14', 'KRT7', 'EPCAM', 'IGHG2', 'IGKC', 'IGHA1',
               'HNF1B', 'ANXA2', 'FLNA', 'ANXA11', 'ALB', 'FABP1', 'APOB', 'APOE', 'A2M', 'APOC2')) {
  print(gene)
  
  fig <- lapply(c(1:6), function(ids){
  ...
  jpeg(file=paste0(res_folder,Sys.Date(),"_",gene, ".jpeg"),width=960,height=960,quality = 100)
  grid.arrange(grobs=fig)
  dev.off()
}

# separate MAST up and dn significant genes =====================================================
files= grep("MAST_", list.files(res_folder), value = T)
for(ids in c(1:length(spatial_list))){
  tmp = read.csv(file.path(res_folder,files[ids]))
  tmp =  tmp[order(tmp[,3],decreasing=FALSE),]
  write.table(tmp[which(tmp$p_val_adj<0.05 & tmp$avg_log2FC>0),], file=file.path(res_folder,paste0("MAST_up_A",ids,".csv")), quote=FALSE, row.names=FALSE, sep="\t")
  write.table(tmp[which(tmp$p_val_adj<0.05 & tmp$avg_log2FC<0),], file=file.path(res_folder,paste0("MAST_dn_A",ids,".csv")), quote=FALSE, row.names=FALSE, sep="\t")
}
